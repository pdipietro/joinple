class SubjectsController < ApplicationController
  before_action :check_social_network, only: %i(show index list)
  before_action :logged_in_subject, only: %i(index edit update show)
  ...
  def list
    logger.debug 'Subject Controller:.list [get subjects/list/:class_name/:object_id/:rel(/from_page(/:limit(/:subject(/:deep))))]'
    ...
    logger.debug "object_id: #{object_id}"
  ...
  def create
    ...
    begin
      ...
      @subject.save
      a_profile = SubjectProfile.new
      a_profile.save
      HasSubjectProfile.create(from_node: @subject, to_node: a_profile)
